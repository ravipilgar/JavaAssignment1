...
public class WeekAsgn {
	static class Node {
		...
		private void sortColumns(int c) throws Exception {
			int emptyCell = this.EmptCellCol(c);
			if (emptyCell != -1) {
				throw new Exception("your column has less than 3 cells filled,");
			}
		...
		private void sortColumnsTwo(int c) throws Exception {
			int emptyCell = this.EmptCellCol(c);
			if (emptyCell == -1) {
				throw new Exception(" your column has 3 cells filled only");
			}
